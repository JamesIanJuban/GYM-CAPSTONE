...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($full_name)) {
        ...
        if ($stmt->fetchColumn() > 0) {
            ...
            // Insert new user
            $stmt = $pdo->prepare("INSERT INTO users (full_name, username, email, password, dob, phone, address, emergency_contact_person, emergency_contact_address, emergency_contact_number, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$full_name, $username, $email, $hashed_password, $dob, $phone, $address, $emergency_contact_person, $emergency_contact_address, $emergency_contact_number, $role]);
...
<body class="bg-gray-900 min-h-screen flex items-center justify-center">
    <form method="POST" class="max-w-lg mx-auto p-8 space-y-4 bg-gray-800 rounded mt-10 text-yellow-400">
        ...
        <input type="date" name="dob" required class="w-full p-3 border rounded bg-gray-900">
